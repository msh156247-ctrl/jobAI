...
CREATE POLICY "Users can view their own profile"
  ...
  USING (id IN (SELECT user_id FROM public.linked_accounts WHERE auth_user_id = auth.uid()));
...
CREATE POLICY "Users can view their own linked accounts"
  ...
  USING (auth_user_id = auth.uid() OR user_id IN (SELECT user_id FROM public.linked_accounts WHERE auth_user_id = auth.uid()));

CREATE POLICY "Users can manage their own linked accounts"
  ON public.linked_accounts FOR ALL
  USING (user_id IN (SELECT user_id FROM public.linked_accounts WHERE auth_user_id = auth.uid()));
...
EXCEPTION
  WHEN unique_violation THEN
    INSERT INTO public.users (username, email, name, avatar_url)
    VALUES (p_username || '_' || substr(md5(random()::text), 1, 6), p_email, p_name, p_avatar_url)
    RETURNING id INTO v_user_id;
    RETURN v_user_id;
END;
...
BEGIN
  ...
  IF v_linked_account_id IS NOT NULL THEN
    ...
    SET user_id = p_user_id, auth_user_id = p_auth_user_id,
        email = p_email, name = p_name, avatar_url = p_avatar_url
    WHERE id = v_linked_account_id;
  ELSE
    INSERT INTO public.linked_accounts (user_id, provider, provider_user_id, auth_user_id, email, name, avatar_url)
    VALUES (p_user_id, p_provider, p_provider_user_id, p_auth_user_id, p_email, p_name, p_avatar_url)
...
RETURNS TABLE (id UUID, username TEXT, email TEXT, name TEXT, avatar_url TEXT, role TEXT, created_at TIMESTAMP WITH TIME ZONE, updated_at TIMESTAMP WITH TIME ZONE)
AS $$
BEGIN
  RETURN QUERY
